...
export function calculateEstimate(
  ...
  const keyOrCommercial = SERVICE_ID_TO_KEY[serviceId];
  if (!keyOrCommercial || keyOrCommercial === 'commercial') {
    return null;
  }

  const serviceKey = keyOrCommercial as ServiceKey;
  const servicePricing = PRICING[serviceKey];
  if (!servicePricing || !sizeId) {
    return null;
  }
